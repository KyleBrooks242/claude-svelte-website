...
The public, DB-backed, read-heavy pages — `/blog`, `/blog/[slug]`, `/projects`, `/projects/[id]` — opt into Vercel ISR via `export const config: Config = { isr: { expiration: 3600, bypassToken: ISR_BYPASS_TOKEN } }` in their `+page.server.ts` (type from `@sveltejs/adapter-vercel`). This exists because the DB runs on low compute (Neon); serving cached responses avoids hitting it on every navigation. Content edited in `/admin` won't show up on these routes until the cache expires (1hr) *or* the admin "Clear cache" button is used — `src/lib/cache.ts` (`revalidateCachedPages`) hits every published post/project URL with `x-prerender-revalidate: <token>` to force Vercel to regenerate them immediately. `ISR_BYPASS_TOKEN` must be the same value in `.env.local` and in Vercel's project env vars, or the bypass requests will silently fail to match.
