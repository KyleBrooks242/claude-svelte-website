...
The public, DB-backed, read-heavy pages — `/blog`, `/blog/[slug]`, `/projects`, `/projects/[id]` — opt into Vercel ISR via `export const config = isrConfig` in their `+page.server.ts`, where `isrConfig` (`src/lib/cache.ts`) is a shared `{ isr: { expiration: 24h, bypassToken: ISR_BYPASS_TOKEN } }` config. This exists because the DB runs on low compute (Neon); serving cached responses avoids hitting it on every navigation. The 24h expiration is a staleness ceiling, not the primary freshness mechanism — every admin mutation that touches public content (post/project create, update, delete, and the post publish/unpublish toggle, plus project image add/caption/cover/remove) calls `revalidateCachedPages` and/or `revalidatePath` (`src/lib/cache.ts`) at the end of its action, hitting the affected URL(s) with `x-prerender-revalidate: <token>` so Vercel regenerates them on demand. `revalidateCachedPages` only sweeps the *currently* published posts/projects, so actions that remove something from the public set (unpublish, delete, slug change) must also explicitly `revalidatePath` the specific old URL — otherwise that page keeps serving its last cached (now-wrong) content until the 24h expiration. The admin dashboard's "Clear cache" button calls `revalidateCachedPages` too, as a manual fallback (e.g. if `ISR_BYPASS_TOKEN` was rotated or a revalidation call failed silently). `ISR_BYPASS_TOKEN` must be the same value in `.env.local` and in Vercel's project env vars, or the bypass requests will silently fail to match.
...
All styles live in `src/app.css` — there is no CSS framework. Design tokens are CSS custom properties on `:root`, dark-only (see below). Components use inline styles or scoped `<style>` blocks that reference those variables. Don't introduce a CSS framework without discussion.

**Design concept — "Shop Drawing, printed dark":** the public site's visual identity is a technical/blueprint aesthetic, not a generic SaaS look — a blueprint/photostat reproduced dark (pale lines on a near-black field) and read under a single work lamp. It's grounded in Kyle's two crafts (software + woodworking) both starting from a measured drawing before anything gets built. When adding UI, match this rather than reaching for default rounded-card/Inter/indigo styling:
- **Color** — near-black "vellum" background (`--bg`), ink-warm body text, a single brass/tungsten `--accent` (the lamp), and a redline `--mark` reserved for status/emphasis. Every other token (`--text-muted`, `--status-good`, `--status-warn`, `--status-bad`, `--status-neutral`) is a material state of that same brass at the same low saturation, not a color borrowed at full strength off a wheel — see the token comment in `app.css` before adding a new one. Use the status tokens for any status/PR/health indicator instead of ad hoc hex colors. The site is dark-only: no `[data-theme]` split and no toggle — don't reintroduce a light theme without discussion.
...
- **Signature motif** — a clickable `.card` visibly lifts toward the lamp on hover/focus-within (`translateY`, a warm accent-tinted glow, a slightly lightened surface) rather than staying flat; resting state stays quiet. `.workout-card` and `.pr-card` intentionally don't get this — they aren't clickable, so a hover affordance would be misleading. (An earlier version used corner registration-marks instead of a lift; that was dropped for reading as a UI glitch, and a separate hover-flicker effect was dropped as it read as a rendering issue rather than a deliberate touch — don't reintroduce either without discussion.) `.divider` is a scale-bar (hairline flanked by two end ticks) rather than a plain `<hr>`. `.section-tag` gets a small accent square before the text. Corners are nearly square (`--radius: 3px`), not the rounded SaaS default.
